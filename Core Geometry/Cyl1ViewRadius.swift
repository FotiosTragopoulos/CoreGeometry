...
class Cyl1ViewRadius: UIView {
    ...
    override func draw(_ rect: CGRect)
    {
        ...
        let xView = viewWithTag(35)?.alignmentRect(forFrame: rect).midX
        let yView = viewWithTag(35)?.alignmentRect(forFrame: rect).midY
        
        let width = self.viewWithTag(35)?.frame.size.width
        let height = self.viewWithTag(35)?.frame.size.height
        let size = CGSize(width: width! * 0.63, height: height! * 0.075)
        
        let rectPlacementX = CGFloat(size.width/2)
        let rectPlacementY = CGFloat(size.height/0.227)
        ...
        context?.move(to: CGPoint(x: (xView! - rectPlacementX), y: (yView! - rectPlacementY * 0.9)))
        context?.addLine(to: CGPoint(x: xView!, y: (yView! - rectPlacementY * 0.9)))
