...
class Tri1ViewTriangle: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        let xView = viewWithTag(12)?.alignmentRect(forFrame: rect).midX
        let yView = viewWithTag(12)?.alignmentRect(forFrame: rect).midY
        
        let width = self.viewWithTag(12)?.frame.size.width
        let height = self.viewWithTag(12)?.frame.size.height
