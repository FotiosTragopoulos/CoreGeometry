...
class Tra1ViewTrapezium: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        let xView = viewWithTag(23)?.alignmentRect(forFrame: rect).midX
        let yView = viewWithTag(23)?.alignmentRect(forFrame: rect).midY
        
        let width = self.viewWithTag(23)?.frame.size.width
        let height = self.viewWithTag(23)?.frame.size.height
