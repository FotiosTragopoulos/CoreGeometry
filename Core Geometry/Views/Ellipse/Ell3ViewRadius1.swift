...
class Ell3ViewRadius1: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        let xView = viewWithTag(43)?.alignmentRect(forFrame: rect).midX
        let yView = viewWithTag(43)?.alignmentRect(forFrame: rect).midY
        
        let width = self.viewWithTag(43)?.frame.size.width
        let height = self.viewWithTag(43)?.frame.size.height
        let size = CGSize(width: width! * 0.88, height: height! * 0.5)
