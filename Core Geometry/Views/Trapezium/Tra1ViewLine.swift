...
class Tra1ViewLine: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        let xView = viewWithTag(24)?.alignmentRect(forFrame: rect).midX
        let yView = viewWithTag(24)?.alignmentRect(forFrame: rect).midY
        
        let width = self.viewWithTag(24)?.frame.size.width
        let height = self.viewWithTag(24)?.frame.size.height
