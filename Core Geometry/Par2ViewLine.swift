...
class Par2ViewLine: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        let xView = viewWithTag(4)?.alignmentRect(forFrame: rect).midX
        let yView = viewWithTag(4)?.alignmentRect(forFrame: rect).midY
        
        let width = self.viewWithTag(4)?.frame.size.width
        let height = self.viewWithTag(4)?.frame.size.height
