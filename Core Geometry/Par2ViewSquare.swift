...
class Par2ViewSquare: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        let xView = viewWithTag(3)?.alignmentRect(forFrame: rect).midX
        let yView = viewWithTag(3)?.alignmentRect(forFrame: rect).midY
        
        let width = self.viewWithTag(3)?.frame.size.width
        let height = self.viewWithTag(3)?.frame.size.height
