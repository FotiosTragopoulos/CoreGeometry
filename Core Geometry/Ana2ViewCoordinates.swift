...
class Ana2ViewCoordinates: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        let xView = viewWithTag(49)?.alignmentRect(forFrame: rect).midX
        let yView = viewWithTag(49)?.alignmentRect(forFrame: rect).midY
        
        let width = self.viewWithTag(49)?.frame.size.width
        let height = self.viewWithTag(49)?.frame.size.height
