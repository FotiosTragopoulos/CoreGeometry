...
class Pol1ViewPolygon: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        let xView = viewWithTag(19)?.alignmentRect(forFrame: rect).midX
        let yView = viewWithTag(19)?.alignmentRect(forFrame: rect).midY
        
        let width = self.viewWithTag(19)?.frame.size.width
        let height = self.viewWithTag(19)?.frame.size.height
        ...
        context?.move(to: CGPoint(x: (xView! - linePlacementX), y: yView!))
        context?.addLine(to: CGPoint(x: (xView! - (linePlacementX * 0.5)), y: (yView! - (linePlacementY * 1.3))))
        context?.addLine(to: CGPoint(x: (xView! + (linePlacementX * 0.5)), y: (yView! - (linePlacementY * 1.3))))
