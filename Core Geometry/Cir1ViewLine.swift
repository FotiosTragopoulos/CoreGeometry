...
class Cir1ViewLine: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        let xView = viewWithTag(16)?.alignmentRect(forFrame: rect).midX
        let yView = viewWithTag(16)?.alignmentRect(forFrame: rect).midY
        
        let width = self.viewWithTag(16)?.frame.size.width
        let height = self.viewWithTag(16)?.frame.size.height
