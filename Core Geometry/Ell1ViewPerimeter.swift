...
class Ell1ViewPerimeter: UIView {
    ...
    override func draw(_ rect: CGRect)
    {
        ...
        let xView = viewWithTag(38)?.alignmentRect(forFrame: rect).midX
        let yView = viewWithTag(38)?.alignmentRect(forFrame: rect).midY
        
        let width = self.viewWithTag(38)?.frame.size.width
        let height = self.viewWithTag(38)?.frame.size.height
