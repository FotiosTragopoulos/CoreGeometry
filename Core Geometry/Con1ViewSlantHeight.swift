...
class Con1ViewSlantHeight: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        let xView = viewWithTag(31)?.alignmentRect(forFrame: rect).midX
        let yView = viewWithTag(31)?.alignmentRect(forFrame: rect).midY
        
        let width = self.viewWithTag(31)?.frame.size.width
        let height = self.viewWithTag(31)?.frame.size.height
        ...
        context?.addLine(to: CGPoint(x: (xView! - linePlacementX * 0.96), y: (yView! + linePlacementY * 1.2)))
