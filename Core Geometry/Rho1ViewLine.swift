...
class Rho1ViewLine: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        let xView = viewWithTag(27)?.alignmentRect(forFrame: rect).midX
        let yView = viewWithTag(27)?.alignmentRect(forFrame: rect).midY
        
        let width = self.viewWithTag(27)?.frame.size.width
        let height = self.viewWithTag(27)?.frame.size.height
        ...
        context?.addLine(to: CGPoint(x: (xView! * 1.2), y: (yView! + (linePlacementY * 0.7))))
