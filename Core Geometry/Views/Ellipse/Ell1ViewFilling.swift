...
class Ell1ViewFilling: UIView {
    ...
    override func draw(_ rect: CGRect)
    {
        ...
        let xView = viewWithTag(37)?.alignmentRect(forFrame: rect).midX
        let yView = viewWithTag(37)?.alignmentRect(forFrame: rect).midY
        
        let width = self.viewWithTag(37)?.frame.size.width
        let height = self.viewWithTag(37)?.frame.size.height
