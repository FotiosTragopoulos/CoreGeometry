...
class Par1ViewLine: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        let xView = viewWithTag(2)?.alignmentRect(forFrame: rect).midX
        let yView = viewWithTag(2)?.alignmentRect(forFrame: rect).midY
        
        let width = self.viewWithTag(2)?.frame.size.width
        let height = self.viewWithTag(2)?.frame.size.height
