...
class Ana1ViewCoordinates: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        let xView = viewWithTag(46)?.alignmentRect(forFrame: rect).midX
        let yView = viewWithTag(46)?.alignmentRect(forFrame: rect).midY
        
        let width = self.viewWithTag(46)?.frame.size.width
        let height = self.viewWithTag(46)?.frame.size.height
