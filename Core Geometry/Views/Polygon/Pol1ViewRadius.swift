...
class Pol1ViewRadius: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        let xView = viewWithTag(21)?.alignmentRect(forFrame: rect).midX
        let yView = viewWithTag(21)?.alignmentRect(forFrame: rect).midY
        
        let width = self.viewWithTag(21)?.frame.size.width
        let height = self.viewWithTag(21)?.frame.size.height
        let size = CGSize(width: width! * 0.6, height: height! * 0.4)
        
        let linePlacementX = CGFloat(size.height/1.6)
        
        context?.move(to: CGPoint(x: xView!, y: yView!))
        context?.addLine(to: CGPoint(x: (xView! + linePlacementX * 1.5), y: yView!))
