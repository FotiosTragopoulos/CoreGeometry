...
class Ana3ViewCoordinates_Z: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        let xView = viewWithTag(53)?.alignmentRect(forFrame: rect).midX
        let yView = viewWithTag(53)?.alignmentRect(forFrame: rect).midY
        
        let width = self.viewWithTag(53)?.frame.size.width
        let height = self.viewWithTag(53)?.frame.size.height
        ...
        context?.addLine(to: CGPoint(x: (xView! - linePlacementX * 0.5), y: (yView! + linePlacementY * 1.7)))
