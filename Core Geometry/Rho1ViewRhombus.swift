...
class Rho1ViewRhombus: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        let xView = viewWithTag(26)?.alignmentRect(forFrame: rect).midX
        let yView = viewWithTag(26)?.alignmentRect(forFrame: rect).midY
        
        let width = self.viewWithTag(26)?.frame.size.width
        let height = self.viewWithTag(26)?.frame.size.height
