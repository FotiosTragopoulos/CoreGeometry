...
class Ell3ViewRadius2: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        let xView = viewWithTag(44)?.alignmentRect(forFrame: rect).midX
        let yView = viewWithTag(44)?.alignmentRect(forFrame: rect).midY
        
        let width = self.viewWithTag(44)?.frame.size.width
        let height = self.viewWithTag(44)?.frame.size.height
        let size = CGSize(width: width! * 0.88, height: height! * 0.595)
