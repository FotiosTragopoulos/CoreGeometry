...
class Ell3ViewRadius3: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        let xView = viewWithTag(45)?.alignmentRect(forFrame: rect).midX
        let yView = viewWithTag(45)?.alignmentRect(forFrame: rect).midY
        
        let width = self.viewWithTag(45)?.frame.size.width
        let height = self.viewWithTag(45)?.frame.size.height
        let size = CGSize(width: width! * 0.6, height: height! * 0.1)
